...
async def generate_roadmap(topic: str) -> RoadmapResponse:
    ...
    try:
        ...
        # 为每个node自动匹配最相关的source_url
        nodes = []
        for node in roadmap_data["nodes"]:
            matched_url = ""
            for src in sources:
                # 简单规则：如果node标题的关键词在source标题或摘要中出现
                node_keywords = node["title"].split()
                if any(kw in src.title or kw in src.snippet for kw in node_keywords):
                    matched_url = src.url
                    break
            node["source_url"] = matched_url
